...
BazookaTower::BazookaTower(std::string _path, sf::Vector2f pos, sf::Vector2f resize):Tower()
{
...
BazookaTower::~BazookaTower()
{
    delete this->bullet;
}


void BazookaTower::Update(const sf::Vector2f mousePos, const float& dt){
    wait += dt;
    // bullet animation
...
void BazookaTower::Render(sf::RenderTarget* window){
    window->draw(range);
    window->draw(sprite);
    if(this->isSomeEmemy){
...
void BazookaTower::rotate(sf::Vector2i mousePos, sf::Vector2f pos){
...
void BazookaTower::enemieInRange(std::vector<Enemy*> enemies){
    int j = 0;
    this->isSomeEmemy = false;
    for(auto i: enemies){
...
void BazookaTower::collisionDetect(std::vector<Enemy*> enemies, float dt){
    for(auto i: enemies){
